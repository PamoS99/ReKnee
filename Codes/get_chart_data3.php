...
while ($countdayyss < 8) {
    ...
    while ($rowmss = mysqli_fetch_array($sqlmss)) {
        ...
        if ($rowmsst > 0) {
            $angleecalin4 = $rowmsst['angle2'];
        }
        $endtimee = $rowmss['endtime'];
        $starttimee = $rowmss['starttime'];
        $sqlmsstm = mysqli_query($con, "SELECT max(abs(angle2- '$angleecalin4')) as maxid FROM currentangle WHERE datetime>='$starttimee' and datetime<='$endtimee'");
