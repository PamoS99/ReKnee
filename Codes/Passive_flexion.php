...
if (isset($_POST['continbtn'])) {
    $sqlm = mysqli_query($con, "SELECT * FROM currentexercise WHERE user_id='1' and excersie_id='2'");
    ...
    if ($rowm > 0) {
        ...
        $sqls = mysqli_query($con, "update currentexercise set status='0' WHERE user_id='1'");
        $sqls = mysqli_query($con, "insert into currentexercise (status,user_id,excersie_id) VALUES (1, 1, 2)");
    }
    $currentDate = date('Y-m-d');
    $countdayyss= $_SESSION['selectedDay'];
    $sqlmss = mysqli_query($con, "SELECT * FROM repcountset WHERE user_id='1' and excersie_id='2' and daynumber='$countdayyss'");
    $rowmss = mysqli_fetch_array($sqlmss);
    if ($rowmss > 0) {
        
    } else {
        $sqlmssa = mysqli_query($con, "SELECT * FROM repcountset WHERE user_id='1' and excersie_id='2' order by id desc limit 1");
        $rowmssa = mysqli_fetch_array($sqlmssa);
        if ($rowmssa > 0) {
            $sqls = mysqli_query($con, "update repcountset set status='0' WHERE user_id='1' and excersie_id='2'");
            ...
            $sqls = mysqli_query($con, "insert into repcountset (setnumber,daynumber,datetime,status,excersie_id,user_id) VALUES (1, '$countdayy','$currentDate',1,2,1)");
        } else {
            $countdayy= $_SESSION['selectedDay'];
            $sqls = mysqli_query($con, "update repcountset set status='0' WHERE user_id='1' and excersie_id='2'");
            $sqls = mysqli_query($con, "insert into repcountset (setnumber,daynumber,datetime,status,excersie_id,user_id) VALUES (1, '$countdayy','$currentDate',1,2,1)");
    ...
    $sqlmssr = mysqli_query($con, "SELECT * FROM repcountset WHERE user_id='1' and excersie_id='2' order by id desc limit 1");
    $rowmssr = mysqli_fetch_array($sqlmssr);
    if ($rowmssr > 0) {
        $idddd=$rowmssr['id'];
        $sqlmssat = mysqli_query($con, "SELECT * FROM repcheckcount WHERE user_id='1' and excersie_id='2' and repcountset_id='$idddd' order by id desc limit 1");
        $rowmssat = mysqli_fetch_array($sqlmssat);
        if ($rowmssat > 0) {
            
        }else{
            $sqls = mysqli_query($con, "update repcheckcount set status='0' WHERE user_id='1' and excersie_id='2'");
            $sqls = mysqli_query($con, "insert into repcheckcount (count,status,excersie_id,user_id,repcountset_id) VALUES (0, 1,2,1,'$idddd')");
        }
    }
    header('location:Passive_flexion_dashboard.php');
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="container-fluid pt-4 px-4">
                ...
                <div class="col-sm-12 col-lg-6 pb-3"> 
                    ...
                    <div class="bg-light rounded d-flex align-items-center justify-content-center p-2"> 
                        <h5 class=" col-12 col-sm-6 text-center">Passive Flexion</h5>           
                    </div>
                    <div class="bg-light position-relative">
                        <img  class=" d-flex justify-content-center" src="img/PassiveFlexion.png" alt="" style="width:75%; height: auto ;margin: auto"></img>                       
                    </div>
                    ...
                    <div class="bg-light rounded d-flex align-items-justify p-1 ">
                        <ol>
                            <li>Do not force the knee into a bend; only go as far as is comfortable.</li>
                            <li> Aim to move from a 90-degree bend to a fully extended position (0 degrees).</li>
                            <li> Ensure each movement is smooth and controlled.</li>
                        </ol>
                    ...
                    <div class="bg-light rounded d-flex align-items-right justify-content p-4">                  
                        <iframe class=" d-flex justify-content-center " allowfullscreen=" "accesskey=""src="https://www.youtube.com/embed/C0z9db2gdG8" style="width:100%; height:200px  ; margin: auto"> 
                        </iframe> 
                    </div>
                    <form method="post">
